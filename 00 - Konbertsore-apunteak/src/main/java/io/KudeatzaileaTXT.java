...
public class KudeatzaileaTXT {
    ...
    public static void txtFitxategianDatuakEguneratu() {
        ...
        fileName = sc.next();
        Filtroak.removeSpaces(fileName);
        path = "Nire_Konbertsorea\\src\\main\\resources\\TXT\\" + fileName + ".txt";
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(path))) {
            ...
            while ((lerroa = br.readLine()) != null) {
                ...
                if (zatiak.length == 2) { // NAN, Adina <-- Txt formatu laburra
                    luzeera = 2;
                    Pertsona p = new Pertsona(zatiak[0], Integer.parseInt(zatiak[1]));
                    pertsonak.add(p);
                } else if (zatiak.length == 5) { // NAN, Izena, Abizena, Adina, Helbidea <-- Txt formatu osoa (CSV-tik
                                                 // TXT-ra bihurtuta)
                    luzeera = 5;
        ...
        // Fitxategian gehitu
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(path, true))) {
            bw.write(p.toCSV()); // CSV formatuan datuak gorde ("," banatuta)
            bw.newLine();
            System.out.println(Gehigarriak.Berdea + "Datuak ondo gehitu dira fitxategian." + Gehigarriak.RESET);
        } catch (IOException e) {
            System.out.println(
